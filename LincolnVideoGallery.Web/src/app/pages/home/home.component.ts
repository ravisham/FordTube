...
export class HomeComponent implements OnInit {
  filter: SearchInterface = {
    categoryIds: this.currentRoute.snapshot.queryParamMap.get('c')
      ? [this.currentRoute.snapshot.queryParamMap.get('c')]
      : ['all'],
    query: encodeURI(this.currentRoute.snapshot.queryParamMap.get('q')) || '',
    sortField: this.getSortField(
      this.currentRoute.snapshot.queryParamMap.get('s')
    ),
    sortDirection:
      this.currentRoute.snapshot.queryParamMap.get('d') === 'desc'
  ...
  getFilter() {
    const searchFilter: SearchInterface = {
      ...
      sortDirection:
        this.currentRoute.snapshot.queryParamMap.get('d') === 'asc'
          ? SortDirectionType.Asc
          : SortDirectionType.Desc,
      tags: this.currentRoute.snapshot.queryParamMap.get('f') === 'true',
    };
  ...
  getSortField(param: string): SortFieldType {
    if (param) {
      switch (param.toLowerCase()) {
        ...
        default:
          return this.router.url.indexOf('category') > 0
            ? SortFieldType.WhenUploaded
            : SortFieldType.Title;
      }
    }
    return this.router.url.indexOf('category') > 0
  ...
  fillVideos() {
    const searchModel: VideoSearchRequestModel = {
      ...
      searchField: this.filter.tags
        ? SearchFieldType.Tags
        : SearchFieldType.All,
    };
  ...
  ngOnInit() {
    ...
    if (
      this.router.url.indexOf('category') > 0 &&
      this.currentRoute.snapshot.queryParamMap.get('c')
    ) {
      this.categoriesService
        .getChildren(this.currentRoute.snapshot.queryParamMap.get('c'))
        .subscribe(
          (response) => {
