...
/**
 * Guard to check if the user is a dealer admin.
 */
export class UserDealerAdminGuard implements CanActivate {
  /**
   * Constructor for UserDealerAdminGuard.
    ...
    return this.authenticationService.validateToken().toPromise().then(response => {
      if (response.valid) {
        this.userProfileService.getUserProfile().subscribe(profile => {
          if (profile.userRoleId === UserRoleEnum.DEALER_ADMIN || profile.userRoleId === UserRoleEnum.SUPER_ADMIN || profile.userRoleId === UserRoleEnum.DEALER) {
            return true;
          }
        });
        return false;
