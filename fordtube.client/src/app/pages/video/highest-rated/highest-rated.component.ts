...
export class HighestRatedComponent implements OnInit {
  countOnPage = 15;
  filter: SearchPanelInterface | undefined;
  ...
  searchVideos(value: SearchPanelInterface) {
    this.videos = [];
    this.filter = value;
    this.scrollId = null;
    this.fillVideos();
  ...
  fillVideos() {
    const searchModel: VideoSearchRequestModel = {
      type: VideoType.All,
      categories: this.filter && this.filter.categoryIds ? this.filter.categoryIds : [],
      uploaders: [],
      uploaderIds: [],
      status: VideoStatus.Active,
      query: this.filter && this.filter.query ? this.filter.query : '', // Set query to an empty string if it is undefined
      count: this.countOnPage,
      titleOnly: false,
      scrollId: this.scrollId || '', // Set scrollId to an empty string if it is null
